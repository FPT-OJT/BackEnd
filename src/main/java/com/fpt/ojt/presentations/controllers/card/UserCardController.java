...
public class UserCardController extends AbstractBaseController {
    ...
    @Operation(
            summary = "Add a new card to user",
            description = "Add a new card to the authenticated user's collection")
    @PostMapping("")
    ...
    @Operation(
            ...
            description = "Retrieve detailed information about a specific user card")
    @GetMapping("/{id}")
    ...
    @Operation(
            summary = "Get user cards by card type",
            description = "Retrieve all user cards filtered by a specific card type")
    @GetMapping("/card-type/{cardType}")
    public ResponseEntity<SingleResponse<List<UserCardDto>>> getUserCardsByCardType(
            @Parameter(description = "Type of card to filter by", required = true, example = "PAYMENT_APP")
                    @PathVariable
                    String cardType) {
    ...
    @Operation(
            summary = "Check if user card exists",
            description = "Verify whether a specific card exists in the user's collection")
