...
public class MerchantServiceImpl implements MerchantService {
    ...
    @Override
    public List<HomePageResponse.MerchantOffer> getMerchantOffers(
            int limit, UUID currentUserId, Coordinate userLocation) {
                            ...
                            .map(availableDeal -> executor.submit(() -> {
                                var isFavoriteFuture = executor.submit(
                                        () -> favoriteMerchantRepository.existsByUserIdAndMerchantAgencyId(
                                                currentUserId, agency.getMerchantAgencyId()));
                                ...
                                var totalDiscountFuture = executor.submit(
                                        () -> dealService.calculateMerchantOfferOnMerchantDealAndUserCardList(
                                                agency.getMcc(), availableDeal, availableCardRulesDtos));
    ...
    private List<MerchantOfferDto> getOffersWithoutMerchants(UUID currentUserId) {
        ...
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            var offerFutures = availableCardRulesDtos.stream()
                    .flatMap(cardDto -> cardDto.getCardRules().stream().flatMap(cardRule -> {
                        ...
                        // Case 1: allowMccs == null && rejectMccs == null → all
                        // merchants
                            ...
                            return agencies.stream()
                                    .map(agency -> executor.submit(() -> {
                                        ...
                                        return MerchantOfferDto.builder()
                                                ...
                                                .imageUrl(agency.getImageUrl())
                                                .merchantDealName(
                                                        "Card Benefit: " + String.format("%.1f%%", cardBenefit))
                                                .isFavorite(isFavorite)
                                                .isSubscribed(isSubscribed)
                        ...
                        // Case 2: allowMccs == null && rejectMccs != null →
                        // Allow all except rejected
                        if (allowMccs == null && !rejectMccs.isEmpty()) {
                            List<MerchantAgency> agencies =
                                    merchantAgencyRepository.findAllActiveExcludingMccs(rejectMccs);
                        ...
                        if (allowMccs != null && !allowMccs.isEmpty()) {
                            return allowMccs.stream().flatMap(mcc -> {
            ...
            return merchantOfferDtos.stream()
                    .collect(java.util.stream.Collectors.toMap(
                            ...
                            offer -> offer,
                            (existing, replacement) -> existing.getTotalDiscount() >= replacement.getTotalDiscount()
                                    ? existing
                                    : replacement))
                    .values()
                    .stream()
