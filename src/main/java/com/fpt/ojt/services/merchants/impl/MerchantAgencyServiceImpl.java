...
public class MerchantAgencyServiceImpl implements MerchantAgencyService {
    ...
    private NearestAgencyDto mapToNearestAgencyDto(
        NearestAgencyProjection projection
    ) {
    ...
    public List<MerchantDealDto> findAvailableDeals(UUID merchantAgencyId) {
        var deals = merchantDealRepository.findAvailableByMerchantAgencyId(
            merchantAgencyId
        );
        return deals.stream().map(this::mapToMerchantDealDto).toList();
    }

    private MerchantDealDto mapToMerchantDealDto(
        com.fpt.ojt.models.postgres.deal.MerchantDeal deal
    ) {
        return MerchantDealDto.builder()
            ...
            .merchantDescription(
                deal.getMerchantAgency().getMerchant().getDescription()
            )
